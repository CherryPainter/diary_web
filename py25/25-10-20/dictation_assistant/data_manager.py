...
def reset_all_data(base_dirs=None):
    ...
    targets = {
        ...
        # module_stats.json 由 logger 管理，但我们也在磁盘上重置一个副本
        'module_stats.json': {
            "E2C": {"total": 0, "correct": 0, "wrong": 0},
            "C2E": {"total": 0, "correct": 0, "wrong": 0},
            "LISTEN": {"total": 0, "correct": 0, "wrong": 0},
    ...
    # 如果 logger 提供了保存接口，让 logger 重置模块统计并持久化
    try:
        if hasattr(logger, 'module_stats'):
            for k in list(logger.module_stats.keys()):
                logger.module_stats[k] = {"total": 0, "correct": 0, "wrong": 0}
