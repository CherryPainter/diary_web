...
from data_manager import adjust_word_weight, meaning_matches
from voice_processor import speak, recognize_speech
# 导入日志模块
try:
    from logger import logger
except ImportError:
    print("警告：在app.py中无法导入日志模块")
...
class DictationApp:
    ...
    def _create_history_page(self):
        ...
        try:
            from data_manager import get_history_records
            # 获取更多天数的历史记录，显示最近30天
    ...
    def _create_stats_page(self):
        """创建统计页面"""
        # 导入logger模块以获取会话统计数据（若不可用，使用 None）
        try:
            from logger import logger
        except ImportError:
            logger = None
    ...
    def reset_stats(self):
        ...
        # 统一使用 data_manager 的重置函数来清空所有持久化数据
        try:
            from .data_manager import reset_all_data
            written, failed = reset_all_data()
            logger.log(f"reset_all_data wrote: {written}, failed: {failed}")
        except Exception as e:
            logger.log(f"调用 reset_all_data 失败: {e}")
        ...
        messagebox.showinfo("成功", "统计数据、单词统计、历史学习记录和错题记录已全部重置")
    ...
    def update_stats(self, is_correct):
        ...
        try:
            from logger import logger
            # logger 的模块键使用小写 'review'，但 app 中 mode 使用 'REVIEW'
        ...
        # 更新每日学习进度
        try:
            from data_manager import update_today_progress
        ...
        if hasattr(self, 'todo_progress_var'):
            try:
                ...
                self.todo_progress_var.set(today_goal["percentage"])
                if hasattr(self, 'todo_current_label'):
                    self.todo_current_label.config(text=str(today_goal["current"]))
                if hasattr(self, 'todo_target_label'):
                    self.todo_target_label.config(text=str(today_goal["target"]))
            except Exception:
                pass
        # 确保统计页面的其它部分也会刷新（如果已创建）
        try:
            self._update_stats_data()
        except Exception:
            pass
    ...
    def exit_app(self):
        ...
        if messagebox.askyesno("确认退出", "确定要退出英语听写助手吗？"):
            # 确保最终保存一次每日学习统计数据
            try:
                from data_manager import save_daily_goals, load_daily_goals
                # 重新加载并保存一次，确保数据完整性
    ...
    def check_answer(self):
        ...
        if is_correct:
            ...
            try:
                ...
                pass
            # 记录答题日志
            module = self.mode if self.mode != "REVIEW" else "review"
            logger.log_answer(module, True, self.word)
            ...
            # 记录答题日志
            module = self.mode if self.mode != "REVIEW" else "review"
            logger.log_answer(module, False, self.word)
